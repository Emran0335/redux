...
export default function Description({ video }) {
  ...
  // we will not find the home page updated if the delete action occured. So, we neet to render application again and so we need to use useEffect hook
  useEffect(() => {
    if (isSuccess) {
      navigate("/");
    }
    ...
    <div>
      ...
      <div className="pb-4 flex items-center space-between border-b gap-4">
        ...
        <div className="flex gap-6 w-full justify-end">
          ...
          <div className="flex gap-1 cursor-pointer" onClick={handleDelele}>
            <div className="shrink-0">
      ...
      {!isLoading && isError && (
        <Error message="There was an error deleting the vedio!" />
      )}
    </div>
  );
}
